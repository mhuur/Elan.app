...
- **Formulaires épurés** (demande utilisateur, juin 2026) : les fiches séance/exercice n'affichent plus de murs de chips — catégorie en `Select`, section du planning et sous-types via `Combobox` (`src/components/ui.tsx` : saisie → filtre (label OU groupe) → « + Créer » en fin de liste si aucune correspondance exacte ; en-têtes de groupes via `option.group`, panneau en z-50 au-dessus de la barre d'action). Les exercices d'une séance s'ajoutent/créent par la même combobox ; chaque item = carte blanche compacte avec saisie directe `MiniNum` (séries × cible + bascule reps/sec + repos). Le formulaire de séance n'édite plus `metrics`/`links`/`notes` : ils sont **préservés tels quels** à la sauvegarde (le vélo retombe sur `DEFAULT_VELO_METRICS` via `effectiveMetrics`) et toujours affichés dans CompleteSheet s'ils existent. Boutons Enregistrer/Dupliquer/Supprimer regroupés dans `FormActions` (barre fixe en bas) ; la BottomNav est masquée sur `/session/*` et `/exercise/*` (`formScreen` dans App.tsx).

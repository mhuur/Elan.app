...
- Objectifs : `Exercise.goal` (meilleure série/volume muscu) et `Session.objective.targets[]` (multi-mesures, toutes à remplir dans une même séance — normalisé par `objectiveTargets()` dans lib/metrics), gérés dans l'onglet `/goals`, célébrés à la complétion (CompleteSheet).

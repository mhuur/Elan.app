// Lit les courses importées (activities) + logs depuis Firestore, lecture seule.
// Variante de fetch-logs.mjs pour l'analyse coaching. Usage : node scripts/fetch-activities.mjs
...
async function collectionGroup(colId) {
  const out = []
  const q = { structuredQuery: { from: [{ collectionId: colId, allDescendants: true }], limit: 2000 } }
...
const activities = await collectionGroup('activities')
const logs = await collectionGroup('logs')
console.log(JSON.stringify({ nActivities: activities.length, nLogs: logs.length, activities, logs }, null, 1))
